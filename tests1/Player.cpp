...
void Player::find_pre_post(std::shared_ptr<Node<std::shared_ptr<Player>, PlayerByRank>> root, std::shared_ptr<Player>& player,
                           std::shared_ptr<Node<std::shared_ptr<Player>, PlayerByRank>>& pre, std::shared_ptr<Node<std::shared_ptr<Player>,
    ...
    if (root->value->get_id() == player->get_id()) {
        ...
        if (root->left != nullptr) {
            std::shared_ptr<Node<std::shared_ptr<Player>, PlayerByRank>> tmp = root->left;
            while (tmp->right)
                tmp = tmp->right;
            pre = tmp;
        }
        ...
        if (root->right != nullptr) {
            std::shared_ptr<Node<std::shared_ptr<Player>, PlayerByRank>> tmp = root->right;
            while (tmp->left)
                tmp = tmp->left;
            post = tmp;
        }
...
void Player::calc_closest(std::shared_ptr<Node<std::shared_ptr<Player>, PlayerByRank>> root, std::shared_ptr<Player>& player) {
    std::shared_ptr<Node<std::shared_ptr<Player>, PlayerByRank>> previous = nullptr, post = nullptr;
...
void Player::setTeamId(int n_id) {
    teamId = n_id;
}

void Player::set_team(std::shared_ptr<Team> t){
    team = t;
}
